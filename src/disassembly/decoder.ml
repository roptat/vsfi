...
type rentry = Elf_info.Reloc.rentry

module type T =
sig
  (** from the bitstring starting at the beginning of the function and a list
      of relocations, the offset at which the function starts, and at which it
      finishes, the initial context and the initial IP, return the
      associated decoded program. *)
  val prog_from_bitstring: bitstring -> rentry IntMap.t -> int -> int -> < value_of_register : Register.t -> Z.t; .. > -> Data.Address.t -> prog
  val esp: Register.t
end
...
struct
    ...
    type state = {
        ...
        relocs            : rentry IntMap.t; (** List of relocations *)
      ...
      mutable ret: bool;             (** true whenever a RET opcode has been decoded *)
    }
    ...
    let find_reloc_at relocs addr =
        IntMap.fold (fun key reloc acc -> if reloc.Elf_info.Reloc.r_offset = addr then (Some reloc) else acc) relocs None

    (** helper to get immediate of _imm_sz_ bits into a _sz_ Const, doing
        _sign_ext_ if true*)
    let get_imm s imm_sz sz sign_ext =
        let addr = (Int32.of_int ((Z.to_int (Address.to_int s.a)) + (List.length s.c))) in
        match find_reloc_at s.relocs addr with
        | None -> const_of_Z (get_imm_int s imm_sz sz sign_ext) sz
        | Some reloc ->
          begin
            let value = (get_imm_int s imm_sz sz sign_ext) in
            let relocstr = (Elf_info.Reloc.string_of_rentry reloc) in
            L.debug (fun p -> p "%s replaced with relocation %s" (Z.to_string value) relocstr);
            Asm.Reloc (Asm.Rentry (reloc, (Data.Word.of_int value (Elf_info.Reloc.size_of_rtype reloc.Elf_info.Reloc.r_type))))
          end
    ...
    let return s stmts =
        stmts, Address.add_offset s.a (Z.of_int s.o)
    ...
    let relative s off_sz sz =
        let delta = get_imm s off_sz sz true in
        match delta with
        | Const delta ->
          begin
            let a' = Address.add_offset s.a (Z.add (Z.of_int s.o) (Data.Word.to_int delta)) in
            check_jmp s a';
            (A a')
          end
        | Reloc r ->
        ...
        | _ -> failwith "Impossible return value from get_imm"
    ...
    let loop s c =
        ...
        in
        return s [ dec_stmt ; If (cond, [Jmp a'], [ Jmp (A (Address.add_offset s.a (Z.of_int s.o)))]) ]
    ...
    let mov_with_eax s n from =
        (* TODO: Check 32*)
        let imm = get_imm s 32 32 false in
        let leax = V (to_reg eax n) in
        let lmem = M (add_segment s imm s.segments.data, n) in
        let dst, src =
    ...
    let shift_l_stmt dst sz n =
        ...
        let cf_stmt =
            ...
            If (c,
                ...
                [Set (V (T fcf), UnOp ((ZeroExt 1), (BinOp (And, one_sz, (BinOp(Shr, ldst, BinOp(Sub, sz', n_masked)))))))])
        in
        let of_stmt =
    ...
    let rotate_r_stmt dst sz count =
      ...
      let cf_stmt = Set (V (T fcf), UnOp((ZeroExt 1), msb)) in 
      let of_stmt = If (Cmp (EQ, count_masked, one),
    ...
    (** jump statement at the end of the return instruction.
        pop already occured, so we read 4 bytes below that to find the return address. *)
    let jump_of_ret s =
      let four = Asm.Const (Data.Word.of_int (Z.of_int 4) 32) in
      let esp = Asm.Lval (Asm.V (Asm.T esp)) in
      Asm.Jmp (Asm.R (Asm.Lval (Asm.M ((Asm.BinOp (Asm.Sub, esp, four)), s.addr_sz))))
    ...
    let decode s =
        ...
        let rec decode s =
            ...
            | '\x9a' -> (* CALL *)
              ...
              let off = get_imm s s.operand_sz s.operand_sz false in
              begin
              match off with
              | Const off ->
                let cs' = get_base_address s (Hashtbl.find s.segments.reg cs) in
                let a = Data.Address.add_offset (Data.Address.of_int Data.Address.Global cs' s.addr_sz) (Data.Word.to_int off) in
                call s (A a)
              | Reloc r ->
                call s (R off)
              | _ -> failwith "Impossible return from get_imm"
              end
            ...
            | c when '\xb8' <= c && c <= '\xbf' -> (* mov immediate word or double into word or double register *)
              let r = (find_reg_v ((Char.code c) - 0xb8) s.operand_sz) in return s [Set (r, get_imm s s.operand_sz s.operand_sz false)]

            | '\xc0' -> (* shift grp2 with byte size*) grp2 s 8 None
            | '\xc1' -> (* shift grp2 with word or double-word size *) grp2 s s.operand_sz None
            | '\xc2' -> (* RET NEAR and pop word *) s.ret <- true; return s [ (* pop imm16 *)  set_esp Add (T esp) (s.addr_sz + 16); (jump_of_ret s)]
            | '\xc3' -> (* RET NEAR *) s.ret <- true; return s [ set_esp Add (T esp) s.addr_sz; (jump_of_ret s) ]
            ...
            | '\xca' -> (* RET FAR *) return s ([set_esp Add (T esp) s.addr_sz; ] @ (pop_stmts s [V (T cs)]) @ [jump_of_ret s])
            | '\xcb' -> (* RET FAR and pop a word *) return s ([set_esp Add (T esp) s.addr_sz ; ] @ (pop_stmts s [V (T cs)] @ (* pop imm16 *) [set_esp Add (T esp) 16] @ [jump_of_ret s]))
              ...
              if s.ret then
                ...
                | _ ->
                   ...
                   if not (s.repe || s.repne) then
                     (*v.Cfa.State.stmts <-*) [ Directive (Unroll (Lval (V (T ecx)), 10000)) ] @ blk(**), ip(**)
                   else
                     begin
                       let cmp = if s.repne then EQ else NEQ in
                       let stmts =
                         match (List.hd (List.tl (List.rev s.c))) with
                         | '\xae' -> (unroll_scas cmp s 8)::blk
                         | '\xaf' -> (unroll_scas cmp s s.addr_sz)::blk
                      | _ -> blk
                       in
                       (*v.Cfa.State.stmts <- stmts*) stmts, ip
                     end;
            end(*;
            v, ip*)
    ...
    (** launch the decoder *)
    let parse text is a ctx relocs =
        let s' = {
            a 	       = a;
            o 	       = 0;
            c          = [];
    ...
    let rec prog_from_bitstring bs relocs off len ctx ip =
      let ip_int = (Z.to_int (Data.Address.to_int ip)) in
      if (Z.to_int (Data.Address.to_int ip)) > len then
        IntMap.empty
      else
        bitmatch bs with
        | { func : (len-off)*8 : string } ->
            begin
              match parse func (init ()) ip ctx relocs with
              | None -> failwith "Instruction spans after end of buffer"
              | Some (stmts, nip, seg) ->
                begin
                  let bs = dropbits (((Z.to_int (Data.Address.to_int nip)) - (Z.to_int (Data.Address.to_int ip)))*8) bs in
                  IntMap.add (Z.to_int (Data.Address.to_int ip)) stmts
                             (prog_from_bitstring bs relocs off len ctx nip)
                end
            end
        | { _ } -> failwith "File too short"
