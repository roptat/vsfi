...
void * allocvector(size_t size)
{
  ...
  if ((V = (void *) malloc((size_t) size)) == (void *) 0) {
    __builtin_debug(6, "", 1);
             ...
             double eps1, double * eps2, int * z, double * x)
{
  ...
  xmin = c[n - 1] - (double) 1.01E0 * fabs(b[n - 1]);
  __builtin_debug(6, "", 1);
  __builtin_debug(1, "#line:bisect.c:249");
  xmax = c[n - 1] + (double) 1.01E0 * fabs(b[n - 1]);
  __builtin_debug(6, "", 1);
  ...
  for (__builtin_debug(6, "", 1), i = n - 2;
    ...
    h = (double) 1.01E0 * (fabs(b[i]) + fabs(b[i + 1]));
    __builtin_debug(6, "", 1);
  ...
  *eps2 = 0x1.0P-52 * (xmin + xmax > 0 ? xmax : -xmin);
  __builtin_debug(6, "", 1);
  ...
  *z = 0;
  ...
  for (__builtin_debug(6, "", 1, 2), k = m2;
    ...
    while (x0 - xu > 2 * 0x1.0P-52 * (fabs(xu) + fabs(x0)) + eps1) {
      __builtin_debug(6, "", 1, 2);
